package dev.redelegends.player;

import dev.redelegends.database.Database;
import dev.redelegends.database.data.container.*;
...
import dev.redelegends.game.GameTeam;
import dev.redelegends.hook.FriendsHook;
...
public class Profile {
  ...
  private LegendsScoreboard scoreboard;
  ...
  public Profile(String name) throws ProfileLoadException {
    ...
    this.getDataContainer("LegendsCoreProfile", "lastlogin").set(System.currentTimeMillis());
  }
  ...
  public void refresh() {
    ...
    if (!playingGame()) {
      ...
      player.setAllowFlight(player.hasPermission("legendscore.command.fly"));
      this.getDataContainer("LegendsCoreProfile", "role").set(StringUtils.stripColors(Role.getPlayerRole(player, true).getName()));
    }
  ...
  public LegendsScoreboard getScoreboard() {
    return this.scoreboard;
  }
  
  public void setScoreboard(LegendsScoreboard scoreboard) {
  ...
  public double getCoins(String table) {
    DataContainer dataContainer = this.getDataContainer(table, "coins");
    if (dataContainer == null) {
      throw new IllegalStateException("Contêiner de dados não encontrado para: " + table + " coins");
    }
    return dataContainer.getAsDouble();
  }
  ...
  public DeliveriesContainer getDeliveriesContainer() {
    return this.getAbstractContainer("LegendsCoreProfile", "deliveries", DeliveriesContainer.class);
  }

  public PreferencesContainer getPreferencesContainer() {
    return this.getAbstractContainer("LegendsCoreProfile", "preferences", PreferencesContainer.class);
  }
  
  public TitlesContainer getTitlesContainer() {
    return this.getAbstractContainer("LegendsCoreProfile", "titles", TitlesContainer.class);
  }
  
  public BoostersContainer getBoostersContainer() {
    return this.getAbstractContainer("LegendsCoreProfile", "boosters", BoostersContainer.class);
  }
  
  public AchievementsContainer getAchievementsContainer() {
    return this.getAbstractContainer("LegendsCoreProfile", "achievements", AchievementsContainer.class);
  }
  
  public SelectedContainer getSelectedContainer() {
    return this.getAbstractContainer("LegendsCoreProfile", "selected", SelectedContainer.class);
  }

  public DataContainer getDataContainer(String table, String key) {
    Map<String, DataContainer> containerMap = this.tableMap.get(table);
    if (containerMap == null) {
      throw new IllegalStateException("Tabela não encontrada: " + table);
    }
    DataContainer dataContainer = containerMap.get(key);
    if (dataContainer == null) {
      throw new IllegalStateException("Chave não encontrada na tabela " + table + ": " + key);
    }
