package dev.redelegends.servers;

import dev.redelegends.Core;
import dev.redelegends.player.Profile;
import dev.redelegends.plugin.config.LegendsConfig;
import dev.redelegends.servers.balancer.BaseBalancer;
import dev.redelegends.servers.balancer.Server;
import dev.redelegends.servers.balancer.type.LeastConnection;
...
public class ServerItem {
  ...
  public static final LegendsConfig CONFIG = Core.getInstance().getConfig("servers");
  ...
  public String getIcon() {
    return PlaceholderAPI.setPlaceholders(Bukkit.getOnlinePlayers().iterator().next(), this.icon);
  }
