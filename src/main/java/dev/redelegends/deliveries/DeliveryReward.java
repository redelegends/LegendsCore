package dev.redelegends.deliveries;

import dev.redelegends.booster.Booster.BoosterType;
...
public class DeliveryReward {
  ...
  public void dispatch(Profile profile) {
    if (this.type == RewardType.COMANDO) {
      ...
      profile.setStats("LegendsCoreProfile", profile.getStats("LegendsCoreProfile", "cash") + (long) this.values[0], "cash");
    } else if (this.type.name().contains("_COINS")) {
      profile.addCoins("LegendsCore" + this.type.name().replace("_COINS", ""), (double) this.values[0]);
