package dev.redelegends.player.fake;
...
public class FakeManager {
  ...
  private static final LegendsConfig CONFIG = Core.getInstance().getConfig("utils");
  private static final Pattern REAL_PATTERN = Pattern.compile("(?i)legendscorefakereal:\\w*"), NOT_CHANGE_PATTERN = Pattern.compile("(?i)legendscorenotchange:\\w*");
  ...
  public static String replaceNickedChanges(String original) {
    ...
    for (String name : listNicked()) {
      ...
      while (matcher.find()) {
        replaced = replaced.replaceFirst(Pattern.quote(matcher.group()), Matcher.quoteReplacement("legendscorenotchange:" + name));
      }
    }
  ...
  public static String replaceNickedPlayers(String original, boolean toFake) {
    ...
    for (String name : listNicked()) {
      ...
      while (matcher.find()) {
        ...
        backup.add(found.replace("legendscorenotchange:", ""));
        replaced = replaced.replaceFirst(Pattern.quote(found), Matcher.quoteReplacement("legendscorenotchange:" + (backup.size() - 1)));
      }
    ...
    while (matcher.find()) {
      ...
      replaced = replaced.replaceFirst(Pattern.quote(found), Matcher.quoteReplacement(
          fakeNames.entrySet().stream().filter(entry -> entry.getValue().equals(found.replace("legendscorefakereal:", ""))).map(Map.Entry::getKey).findFirst().orElse("")));
    }
    
    matcher = NOT_CHANGE_PATTERN.matcher(replaced);
    while (matcher.find()) {
      String found = matcher.group();
      replaced = replaced.replaceFirst(Pattern.quote(matcher.group()), Matcher.quoteReplacement(backup.get(Integer.parseInt(found.replace("legendscorenotchange:", "")))));
    }
