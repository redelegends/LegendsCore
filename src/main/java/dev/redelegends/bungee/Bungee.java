package dev.redelegends.bungee;

import com.google.common.io.ByteArrayDataOutput;
import com.google.common.io.ByteStreams;
import dev.redelegends.bungee.cmd.Commands;
import dev.redelegends.bungee.listener.Listeners;
...
public class Bungee extends Plugin {
  ...
  public static void sendRole(ProxiedPlayer player, String sound) {
    ...
    player.getServer().sendData("LegendsCore", out.toByteArray());
  }
  ...
  public void onEnable() {
    ...
    getProxy().registerChannel("LegendsCore");
  ...
  public void saveDefaultConfig() {
    for (String fileName : new String[]{"config", "roles", "utils"}) {
      File file = new File("plugins/LegendsCore/" + fileName + ".yml");
      if (!file.exists()) {
  ...
  private void setupRoles() {
    try {
      if (utils.get("fake.role") instanceof String) {
        ...
        YamlConfiguration.getProvider(YamlConfiguration.class).save(utils, new File("plugins/LegendsCore/utils.yml"));
      }
    } catch (IOException ex) {
      ex.printStackTrace();
    }
