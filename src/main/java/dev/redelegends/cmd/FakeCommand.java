...
import static dev.redelegends.player.fake.FakeManager.ALEX;
import static dev.redelegends.player.fake.FakeManager.STEVE;
...
public class FakeCommand extends Commands {
  ...
  public void perform(CommandSender sender, String label, String[] args) {
    ...
    if (!player.hasPermission("legendscore.command.fake") || (label.equalsIgnoreCase("fakel") && !player.hasPermission("legendscore.cmd.fakelist"))) {
    ...
    if (label.equalsIgnoreCase("fake")) {
      ...
      for (int index = 0; index < nicked.size(); index++) {
        sb.append("§c").append(nicked.get(index)).append(" §fé na verdade ").append("§alegendscorefakereal:").append(nicked.get(index)).append(index + 1 == nicked.size() ? "" : "\n");
      }
