...
import dev.redelegends.database.conversor.MongoDBConversor;
...
public class MongoDBDatabase extends Database {
  ...
  public MongoDBDatabase(String mongoURL) {
    ...
    this.tables =
        DataTable.listTables().stream().map(DataTable::getInfo).map(DataTableInfo::name).filter(name -> !name.equalsIgnoreCase("LegendsCoreProfile")).collect(Collectors.toList());
  ...
  public void setupBoosters() {
    if (!Manager.BUNGEE) {
      MongoCollection<Document> collection = this.database.getCollection("LegendsCoreNetworkBooster");
      for (String mg : Core.minigames) {
        if (collection.find(new BasicDBObject("_id", mg)).first() == null) {
          this.executor.execute(() -> collection.insertOne(new Document("_id", mg).append("booster", "nevesbr6").append("multiplier", 1.0).append("expires", 0L)));
        }
      }
    }
  }
  ...
  public void setBooster(String minigame, String booster, double multiplier, long expires) {
    this.executor.execute(() -> this.database.getCollection("LegendsCoreNetworkBooster")
  ...
  public NetworkBooster getBooster(String minigame) {
    try {
      Document document = this.executor.submit(() -> this.database.getCollection("LegendsCoreNetworkBooster").find(new BasicDBObject("_id", minigame)).first()).get();
      if (document != null) {
  ...
  public void openConnection() {
    ...
    this.database = this.client.getDatabase("LegendsCore");
  ...
  public Map<String, Map<String, DataContainer>> load(String name) throws ProfileLoadException {
    ...
    for (DataTable table : DataTable.listTables()) {
      ...
      String prefix = table.getInfo().name().equalsIgnoreCase("legendscoreprofile") ? "" : table.getInfo().name() + ".";
    ...
    if (document != null) {
      ...
      for (String key : document.keySet()) {
        ...
        tableMap.get("LegendsCoreProfile").put(key, new DataContainer(document.get(key)));
      }
    } else {
